...
def f_mc(x):
  ...
  p = 0
  for w in dW:
    s = s0 * np.exp((r - 0.5*sigma*sigma) + sigma*w)
    p = p + np.exp(-r) * np.max([s - e, 0.0])
...
def black_scholes(x):
  ...
  d = (np.log(s0 / K) + (r + 0.5 * sigma ** 2)) / sigma
  n1 = 0.5 * (1.0 + sp.special.erf(d / np.sqrt(2.0)))
  n2 = 0.5 * (1.0 + sp.special.erf((d - sigma) / np.sqrt(2.0)))
  return s0 * n1 - K * np.exp(-r) * n2

config = rsqtoa.create_config(
  cfg=os.path.join(str(current_path), "black_scholes.yml"))

ss_tree = rsqtoa.create_subspace_tree(config, black_scholes)
