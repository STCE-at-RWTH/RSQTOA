...
from rsqtoa.model_wrapper import DataGridModel
from rsqtoa.regressors import RsqtoaRegressor
...
for train_index, test_index in kf.split(range(data.shape[0])):
    ...
    model = DataGridModel(features=features, target=target, lower=lower_limit, upper=upper_limit, partitions=partitions,
                          threshold=threshold)

    regressor = RsqtoaRegressor(config_path=config_path,
                                get_new_approximator_instance=util.get_ann_function_approximator,
                                model=model)
    ...
    df_results = pd.concat([df_results, pd.DataFrame([[fold, training_time, train_mae, train_mse, train_rmse, train_r2, test_mae, test_mse, test_rmse, test_r2]], columns=df_results.columns)], ignore_index=True)
