...
from experiments.commons.regressors import ANNRegressor
...
df_results = pd.DataFrame(
    columns=['fold', 'train_time', 'train_mae', 'train_mse', 'train_rmse',
             'train_r2', 'test_mae', 'test_mse',
             'test_rmse', 'test_r2'])
...
for train_index, test_index in kf.split(range(data.shape[0])):
    ...
    train_x = train_data[feature_variables].astype('float32').values
    train_y = train_data[target_variable].astype('float32').values
    train_y = np.expand_dims(train_y, axis=1)

    regressor = ANNRegressor(input_dimension=len(feature_variables),
